# -*- coding: utf-8 -*-
"""
Created on Thu Aug 19 21:41:38 2021
...
#     ) 
import numpy as np
from . import bounds_pygmo
positions = bounds_pygmo.positions
positionClassifier = bounds_pygmo.positionClassifier
positionPreprocess = bounds_pygmo.positionPreprocess
upperBound = bounds_pygmo.upperBound
lowerBound = bounds_pygmo.lowerBound
vector_support = bounds_pygmo.vector_support
...
class ValuesSearchSpace(object):
    ...
    def _index_function(self, x, dictionary_pos):            
        list_index_techniques_to_use_before = [i for i in positionPreprocess if x[i] > 85]
...
#     new_individuals = newInstance.get_individuals()
...
class IndividuoVector(object):
    ...
    def convert(self, primitive_node_part):
        ...
        if self.contador_classification > 1:
            self.contador_classification = 15 - self.contador_aux
            self.contador_aux += 1
        #print(self.contador_classification)
