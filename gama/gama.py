...
#Function to kill the childs in pygmo_search
...
class Gama(ABC):
    ...
    def __init__(
        ...
        # It is assigning the attributes to the AsyncEvaluator object (class) in __init__
        setattr(
            AsyncEvaluator,
            "__init__",
            partialmethod(
        ...
        self._operator_set = OperatorSet(
            ...
            create_new=partial(
                create_random_expression, #Aqui está porque los individuos son aleatorios
                primitive_set=self._pset,
    ...
    def fit(
        ...
        with self._time_manager.start_activity(
            ...
            self._fixed_pipeline_extension = basic_pipeline_extension(
                # self._fixed_pipeline_extension is a list with imputer
                self._x, is_classification
            )
            #print("self._fixed_pipeline_extension TYPE", type(self._fixed_pipeline_extension[0]))
            #print("self._fixed_pipeline_extension TYPE", self._fixed_pipeline_extension)
            self._operator_set._safe_compile = partial(
                self._operator_set._compile,
                preprocessing_steps=self._fixed_pipeline_extension,
            )
            #print(self._operator_set._safe_compile )
            store_pipelines = (
